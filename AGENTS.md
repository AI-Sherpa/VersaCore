...
- Config keys/values in `package.json`, `tsconfig.json`, etc., env-var names, URLs, error strings, regex patterns
...
VersaCore is a Python library (`pip install versacore`) that provides utilities for interacting with local LLM servers and web scraping. Published to PyPI via GitHub Actions on push to `main`.
...
python -m twine upload dist/*     # Publish to PyPI (CI does this automatically)
```

The GitHub Actions workflow (`.github/workflows/publish.yml`) auto-publishes on push to `main`. Version is in `setup.py`.

## Testing

```bash
pytest                            # Run all tests (tests/ directory is empty currently)
python versacore/llm_chat_api.py  # Manual test — requires LM Studio or Ollama running locally
```

## Architecture

### Installable Package: `versacore/`

The pip-installable library. Modules:

- **`llm_chat_api.py`** — `LLMChatAPI` class: unified interface to local LLM servers (LM Studio on `:1234`, Ollama on `:11434`). Supports streaming with callbacks, retry logic, and both OpenAI-compatible and Ollama response formats.
- **`browserless.py`** — Flask API for web scraping (`/scrape`, `/scrape-tables`). Tries static fetch with BeautifulSoup first, falls back to headless Firefox via Selenium if dynamic content detected. Requires geckodriver.
- **`browserless_v2.py`** — Enhanced version of browserless with timeouts, custom user-agent, and improved table parsing (handles colspan).
- **`webArticleSummariseQuery.py`** — Flask API + CLI to summarize/query web articles. Uses Jina AI (`r.jina.ai`) to fetch article content, then passes to LLM via `LLMChatAPI`. Endpoints: `/summarize` (POST), `/query` (POST).
- **`AWSBedrockLangchain.py`** — AWS Bedrock integration via `langchain_aws.BedrockLLM`. List models and invoke them using AWS CLI profiles.

### Standalone App: `LLMChatApi/`

Contains `VersaCoreLLMChatAPI` in `api.py` — a standalone copy of the LLM chat client (same interface as `versacore/llm_chat_api.py` but with class name `VersaCoreLLMChatAPI`). Not part of the pip package.

### Scraping Scripts: `browserless/`

Standalone scraping scripts (not part of the pip package). Contains Dockerfiles, geckodriver binaries, and task-specific scrapers (`pUOB_*.py`, `pBS.py`, `pGSC.py`, etc.). Has its own conda `environment.yml`.

## Key Patterns

- LLM backends are identified by string: `"lmstudio"` or `"ollama"`. LM Studio uses the OpenAI Python client; Ollama uses raw HTTP requests.
- Streaming is the default for LLM calls. Pass a `callback(chunk, end=False)` function to handle chunks; `end=True` signals completion.
- Web scraping follows a static-first strategy: try `requests` + BeautifulSoup, fall back to Selenium/Firefox if JavaScript-rendered content is detected.
- Flask apps are used for API endpoints; `argparse` is used for CLI mode. Several modules support both.

## Dependencies

Core: `requests`, `openai`, `Flask`, `beautifulsoup4`, `selenium`
AWS module additionally needs: `boto3`, `langchain_aws`
